...
#
# ==========================================================================
#  THIS FILE IS HAND-MAINTAINED. It says "generated" nowhere on purpose.
#
#  `sra export-godot` used to write it, from a template that called itself
#  generated. Fixed 2026-09-02: the exporter now writes
#  `SkyRoadsCamera.gd.generated` beside camera.json and leaves this alone.
#  Before that fix an export replaced everything below with the refuted fit —
#  confirmed by running one, which reports `focal 279.7 px` against the
#  measured 282.7632 here.
#
#  The template holds constants and nothing else: no make_dos_material and so
#  no road shader, no dos_x_scale or dos_y_scale, no ARCH_BAND_RATIOS.
#
#  `tests/test_geometry.gd` still fails by name if any of that is lost, and
#  that tripwire is permanent rather than belt-and-braces: the analysis
#  toolkit is NOT under version control, so an older copy of it — one that
#  still writes this file — can reappear at any time. If it fires:
#  `git checkout -- data/SkyRoadsCamera.gd`, then re-apply the exporter fix.
# ==========================================================================
#
...
## The road in the ORIGINAL's pixel space, which is where the span tables, the
## collision profiles and the arch profiles all count. These were spelled as
## bare numbers in four files before they were named here.
##
## `SkyRoadsPlay` still writes them literally, and deliberately: it is a
## transcription of the binary and reads the way the disassembly reads, so a
## constant there would make the two harder to compare, not easier.
const LANE_W_PX := 0x2E          ## 46 screen px per lane at the ship's depth
const ROAD_LEFT_PX := 0x5F       ## 95: screen x of the road's left edge
## A cell is sliced this finely to build a tunnel bore or a block's bored
## front, so there are 47 slice BOUNDARIES and the lane centre is at 23.
const SLICES_PER_CELL := 46
const SLICE_BOUNDARIES := 47
const CENTRE_SLICE := 23
## Column 3 is the middle lane, so the road's left edge is 3.5 columns out.
const HALF_COLS := 3.5
