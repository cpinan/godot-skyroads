...
#
# ==========================================================================
#  DO NOT RUN `sra export-godot` WITHOUT READING THIS.
#
#  The exporter STILL WRITES this file, from a template that calls itself
#  generated. It has not been generated since 2026-08-30, and that template
#  contains none of what is below: no make_dos_material and so no shader, no
#  dos_x_scale or dos_y_scale, no ARCH_BAND_RATIOS. An export would silently
#  replace every camera result derived since then with the refuted fit, and
#  the first sign would be that the road looks wrong.
#
#  `tests/test_geometry.gd` fails by name if that happens. The fix is
#  `git checkout -- data/SkyRoadsCamera.gd`, and then to stop the exporter
#  writing a file it no longer owns.
# ==========================================================================
#
